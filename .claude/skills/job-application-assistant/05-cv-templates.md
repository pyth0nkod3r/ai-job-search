...
These are role-flavored openers for the master CV. Pick the one that matches the posting;
do not stack or splice them. Each is grounded in the candidate's actual data.
...
> Eka Legal and the Interview Platform, to a Next.js prompt-to-app builder integrated into
...
> project on ultrasonic-sensor navigation for visually-impaired users. Since then I've
> built software (Django/React/Next.js full-stack, agentic AI in production) while
> carrying forward the systems-engineering instincts from the degree and from field
> engineering work on telecom base stations and ultrasonic NDT inspection. Looking for
> roles where the software meets the physical world.

### Template E — Early-career graduate (no specific leaning)

> BEng Electronic and Computer Engineering (Nnamdi Azikiwe University, Awka), now a
> full-stack engineer with three years across software and field engineering. Most
> recent: freelance frontend on the cube-os fintech product (UI, API testing,
> integration), with an agentic prompt-to-app builder shipped as a feature inside it.
> Previously: frontend at GFA Technologies, frontend intern at Sand Technologies via
> ALX Ventures, field service engineering at MP Infrastructure, and ultrasonic NDT at
> Montego Upstream.

## Selection Rule

The drafter picks the template whose opening sentence structure matches the posting's
core ask. Never use a template whose primary claim is unsupported by the role's actual
demands — "agentic" wording in a posting that is a straightforward frontend role is
stretch, and the drafter should drop down to Template A or B. The candidate's strongest
honest claim is "full-stack with hardware/embedded exposure"; the strongest stretch is
"agentic AI engineer", and that should only be used when the posting names agent / LLM /
prompt-to-X work directly.
