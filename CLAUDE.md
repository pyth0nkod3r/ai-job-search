...
- [ ] **CV is exactly 2 pages** - not 1, not 3, *for tailored* CVs (`cv/main_<company>_<role>.tex`). The master at `cv/main_example.tex` may run to 3 pages when the candidate has genuine breadth of signal (the Miracle Anyanwu master is currently 3 pages because the work history and 10+ certifications don't compress without dropping signal — that's an exception, not a permission). Tailored CVs are the rule; the master is the reference.
